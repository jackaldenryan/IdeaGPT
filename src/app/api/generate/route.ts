...
export async function POST(request: Request) {
  try {
    ...
    const finalPrompt = generateFinalPrompt(prompt, parsedIdeas, options.numIdeas);
    ...
    if (!newIdeas) {
      return NextResponse.json({ error: 'No ideas generated' }, { status: 400 });
    }
...
function generateFinalPrompt(prompt: string, ideas: z.infer<typeof Ideas>, numIdeas: number) {
